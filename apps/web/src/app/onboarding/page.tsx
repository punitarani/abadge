...
export default function OnboardingPage(): React.ReactElement | null {
  ...
  async function handleCreatePersonal(): Promise<void> {
    ...
    try {
      ...
      // This handler runs only behind the !userId auth guard below, so userId
      // is non-null here. Pass it so the store records lastUserId and the
      // dashboard's session-user-change scrub won't later clear this org.
      if (userId) {
        useOrgStore.getState().setActiveOrg(userId, {
          id: org.id,
          slug: org.slug,
          name: org.name,
          logo: org.logo ?? null,
  ...
  useEffect(() => {
    ...
    // on first load after this code ships. A spurious clear on a genuinely-
    // fresh store is harmless — the resume-triage list query below re-seeds
    // server truth in the same effect.
