...
import { type approvalStatusEnum, approvals, credentials } from "@abadge/db/schema";
...
export const approvalRoutes = new Hono<Env>()
  ...
  .get("/", async (c) => {
    ...
    if (status) {
      conditions.push(eq(approvals.status, status as (typeof approvalStatusEnum)[number]));
    }
