...
flowchart TB
  ...
  subgraph T4["Tier 4: Remote agents"]
    Agent["Remote agent<br/>Server-managed only<br/>read only"]
  end
...
**Attack surface:**
- Unix domain socket (`0600` permissions, owner-only, inside a `0700` directory)
- Process memory (mitigated by a 15-minute inactivity auto-lock that zeroes key buffers with `Uint8Array.fill(0)`; best-effort only — the JS runtime does not guarantee every copy is erased)
- Swap or core dumps (not mitigated: abadge does not `mlock` key pages)
...
Remote agents (cloud workers, CI runners, webhook handlers) authenticate with short-lived Ed25519 keypair sessions (`abs_`) and can only access **server-managed items** with **`read`** permissions.
...
| XSS in browser | Root key in JS memory only, lost on tab close (no `localStorage`/`IndexedDB`). `secureHeaders()` sets baseline hardening headers. Prefer CLI + daemon for high-security operations. |
