...
Remote agents (cloud workers, CI runners, webhook handlers) authenticate with short-lived Ed25519 keypair sessions (`abs_`) and can only access **server-managed items** with **`reveal_plaintext`** permissions.
