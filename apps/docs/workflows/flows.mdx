...
```mermaid
flowchart TD
  Start["Agent sends request<br/>(Bearer token)"] --> AuthCheck{"Authenticate<br/>token"}
  AuthCheck -->|"abs_ prefix"| SessionAuth["Verify session<br/>(hash lookup, TTL check)"]
  AuthCheck -->|"abl_/abg_ prefix"| KeyAuth["Verify API key<br/>(prefix lookup, hash match)"]
  AuthCheck -->|"other"| LegacyAuth["Legacy Better Auth<br/>API key fallback"]
