...
// under the master ENCRYPTION_KEY. v3 item content is encrypted under the DEK
// (via serverEncrypt with the DEK as the key), so rotating the master key only
// rewraps DEKs — no content re-encryption. See ENVELOPE_SPEC.
...
/**
 * Wrap a profile DEK under the master key (AES-256-GCM). The wrap is bound to
 * `(orgId, profileId)` via `additionalData`, so a wrapped DEK transplanted to
 * another profile fails to unwrap. Returns self-contained base64
...
/**
 * Unwrap a profile DEK wrapped by {@link wrapServerDek}. The `aad` must match the
 * `(orgId, profileId)` the DEK was wrapped under, or unwrap fails authentication.
...
export async function unwrapServerDek(
  ...
  if (dek.byteLength !== DEK_LENGTH) {
    throw new Error(`unwrapped DEK must be ${DEK_LENGTH} bytes, got ${dek.byteLength}`);
  }
  return dek;
}
