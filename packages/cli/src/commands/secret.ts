...
async function secretCreate(args: string[]): Promise<void> {
  ...
  try {
    const result = await client.post<Credential>("/api/credentials", body);
    if (values.json) {
      json(result);
    } else {
...
async function secretList(args: string[]): Promise<void> {
  ...
  try {
    const credentials = await client.get<Credential[]>("/api/credentials");
    if (jsonOutput) {
      json(credentials);
    } else {
      table(
...
async function secretGet(args: string[]): Promise<void> {
  ...
  try {
    ...
    if (values.reveal) {
      ...
      const revealed = await client.get<Credential>(`/api/credentials/${cred.id}?reveal=true`);
      if (values.json) {
        json(revealed);
      } else {
