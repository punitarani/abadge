  ...
  const scoped: ScopedDb = {
    ...
    run(fn) {
      return executor.transaction(async (tx) => {
        // §AB-0011 — set the per-transaction org GUC that the RLS policies read,
        // as the first statement. set_config(_, _, true) is transaction-local (like
        // SET LOCAL) so it survives connection pooling (Hyperdrive RESETs between
        // txns, so a non-transaction-local SET would not hold). Unset => RLS
        // fails closed (zero rows). No-op for superuser/BYPASSRLS connections.
