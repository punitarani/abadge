...
import { agents, items, permissions } from "@abadge/db/schema";
import {
  seedAgent,
  seedAgentSession,
  seedOrg,
...
describe("list cursor pagination (AB-0050)", () => {
  ...
  describe("sub-millisecond cursor precision (AB-0052)", () => {
    ...
    test("agents.list returns all rows when created_at has sub-millisecond precision", async () => {
      const owner = await seedUser(auth);
      const org = await seedOrg(auth, owner.userId);
      const caller = createOperatorCaller(db, auth, owner.headers, org.orgId);

      // Batch-insert N agents with a null public key (no keypair needed for listing).
      const agentValues = Array.from({ length: N }, (_, i) => ({
        id: crypto.randomUUID(),
        organizationId: org.orgId,
        createdBy: owner.userId,
        name: `bulk-agent-${i}`,
      ...
      const allIds = agentValues.map((v) => v.id);

      // Force all agents to the exact same sub-millisecond timestamp.
      await db.execute(
        sql`UPDATE agents SET created_at = ${FIXED_TS}::timestamptz WHERE organization_id = ${org.orgId}`,
      );
      ...
      let pages = 0;
      do {
        const page = await caller.agents.list({ limit: 100, cursor });
        for (const a of page.agents) seen.push(a.id);
        cursor = page.nextCursor ?? undefined;
        pages += 1;
        if (pages > 10) throw new Error("pagination did not terminate");
      } while (cursor);
    ...
    test("items.listForAgent returns all items when created_at has sub-millisecond precision", async () => {
      const owner = await seedUser(auth);
      const org = await seedOrg(auth, owner.userId);
      const agent = await seedAgent(db, { userId: owner.userId, orgId: org.orgId, kind: "remote" });
      const session = await seedAgentSession(db, { agentId: agent.agentId, userId: owner.userId });
      ...
      // Batch-insert N items + N permissions.
      const itemValues = Array.from({ length: N }, (_, i) => ({
        id: crypto.randomUUID(),
        organizationId: org.orgId,
        profileId,
        createdBy: owner.userId,
        label: `bulk-for-agent-${i}`,
        storageMode: "server_managed" as const,
        serverCiphertext: "x",
        serverIv: "x",
        serverKeyVersion: 1,
      ...
      await db.insert(permissions).values(
        itemValues.map((item) => ({
          ...
          itemId: item.id,
          profileId: null,
          capability: "reveal_plaintext" as const,
          grantedBy: owner.userId,
        })),
      );

      const allItemIds = itemValues.map((v) => v.id);
      ...
      let pages = 0;
      do {
        const page = await agentCaller.items.listForAgent({ limit: 100, cursor });
        for (const it of page.items) seen.push(it.id);
        cursor = page.nextCursor ?? undefined;
        pages += 1;
        if (pages > 10) throw new Error("pagination did not terminate");
      ...
      expect(seen.length).toBe(N);
      expect(new Set(seen).size).toBe(N); // no duplicates
      expect([...seen].sort()).toEqual([...allItemIds].sort()); // no gaps
    });
  });
});
