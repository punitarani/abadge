...
* `/onboarding` presents three options:
  * **Personal account** — a one-click action (no name/slug form). `organizations.createPersonal` (no input) auto-generates a workspace name/slug from the user row and transactionally seeds a *personal* org — flagged via `organization.metadata = {"type":"personal"}` (see `PERSONAL_ORG_METADATA` / `isPersonalOrg` in `@abadge/core`) — plus the owner `member` row and a default `server_managed` profile (name and `externalId` both `"default"`). A personal account is presented in the UI as a personal account rather than an organization, but is structurally a normal single-member org: it can hold many agents and the user can still create or join team orgs later (coexistence rides on the existing `X-Abadge-Org-Id` resolution). A personal account is capped at a single profile: org creation seeds the one default `server_managed` profile, and `profiles.create` rejects any further profile on a personal org with `PROFILE_LIMIT_EXCEEDED`. The cap is ≤ 1 (an existence check, not a blanket block), so an admin who deletes the default can recreate exactly one. Team orgs are uncapped. **A personal account is NOT in custody mode.** Because the owner owns every secret, the dashboard frames it as the user's own vault: the overview posture banner, profiles subtitle, and settings copy drop the "on behalf of your users" / "under your custody" framing (centralized in `apps/web/src/lib/workspace-posture.ts`, keyed off `isPersonal` from `organizations.list`/`get` via the `useActiveOrg` hook), member management is hidden in Settings, and the owner can reveal their own item values from the item page (server-managed via `items.ownerReveal`, zero-knowledge via in-browser decrypt). Team organizations stay in custody mode — the dashboard never reveals plaintext there.
