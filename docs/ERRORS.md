...
All Abadge API errors use a consistent JSON envelope:
...
The SDK wraps all API errors in `AbadgeApiError`:
...
try {
  ...
  if (err instanceof AbadgeApiError) {
    ...
    console.error(err.hint);    // actionable hint
    console.error(err.meta);    // structured metadata (may be undefined)
    console.error(err.issues);  // validation issues (VALIDATION_ERROR only)
  }
}
```
...
When accessing multi-field items, inspect `meta.availableFields` to determine which field name to
pass:
...
See [`docs/FIELDS.md`](./FIELDS.md) for the field delivery model.
