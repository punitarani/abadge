...
```json
{
  "server_wrapped_dek": "<base64: iv (12 bytes) || AES-256-GCM(ENCRYPTION_KEY, DEK)>"
}
```

- `server_wrapped_dek` is self-contained: `iv (12) || ciphertext+tag (48 = 32-byte DEK + 16-byte tag)`. The unwrap side splits on the known 12-byte IV length.
- Wrapping uses **no AAD** (the DEK is opaque key material; binding happens at the item layer).
- Populated for `server_managed` profiles on create/bootstrap, or lazily on the first v3 write.
...
1. **DEK wrap round-trip:** a fixed 32-byte `ENCRYPTION_KEY` + fixed 32-byte DEK + fixed 12-byte IV → a committed `server_wrapped_dek` string; unwrapping it returns the exact DEK.
...
3. **Cross-profile isolation:** two profiles wrapping the same DEK material still produce independent DEKs; an item encrypted under profile A's DEK fails authentication when decrypted with profile B's DEK (AB-0030 acceptance #2).
